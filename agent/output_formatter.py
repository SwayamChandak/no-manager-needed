...
def run_output_formatter(state: OpsAgentState) -> dict:
    ...
    explanation_prompt = (
        "Write a clear business operations report in Markdown format. "
        "Every paragraph MUST start with a Markdown heading (## Section Title). "
        "Never write a paragraph without a preceding ## heading. "
        "The heading must describe the section content (e.g. '## What Happened', '## Root Cause Analysis', '## Next Steps'). "
        "After each heading, write the section body as plain prose — no bold, no italic, no bullet points, no backticks.\n\n"
        "Tables are allowed. Use a Markdown pipe table (with a header row and separator row) "
        "when you have 2 or more rows of comparable data, such as:\n"
