...
from tools.actions import apply_discount, create_support_ticket, pause_campaign, restock_product
...
def run_action_executor(state: OpsAgentState) -> dict:
    """
    Action executor node — calls the appropriate tool for each approved action.
    Only reached after HITL approval.
    """
    ...
    for action in approved_actions:
        try:
            params = json.loads(action.parameters) if isinstance(action.parameters, str) else action.parameters
